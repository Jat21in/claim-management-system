...
export class PlanDetailComponent implements OnInit, AfterViewInit {
  ...
  activeTab: 'overview' | 'coverage' | 'analytics' | 'exclusions' | 'addons' = 'overview';
  ...
  // Workflow steps
  workflowSteps = [
    { step: 1, title: 'Select Plan', description: 'Choose your coverage', completed: true, active: false },
    { step: 2, title: 'Complete KYC', description: 'Verify identity', completed: false, active: false },
    { step: 3, title: 'Make Payment', description: 'First premium', completed: false, active: false },
    { step: 4, title: 'Policy Active', description: 'Coverage starts', completed: false, active: false }
  ];
    ...
    this.claimsChart = new Chart(ctx, {
        ...
        datasets: [
          {
            ...
            data: this.claimsHistory.map(d => d.amount / 100000),
            borderColor: '#22D3EE',
      ...
      options: {
        ...
        scales: {
          y: {
            grid: { color: '#1F2937' },
            ticks: { color: '#9CA3AF', callback: (value: number | string) => `₹${value}L` },
            title: { display: true, text: 'Claim Amount', color: '#9CA3AF' }
          },
          y1: {
            position: 'right',
            grid: { display: false },
            ticks: { color: '#A78BFA' },
            title: { display: true, text: 'Number of Claims', color: '#A78BFA' }
          },
          x: {
            grid: { display: false },
            ticks: { color: '#9CA3AF' }
  ...
  getPlanDetail(): PlanDetail | null {
    ...
    return this.planDetails[this.currentPlan.name] || this.planDetails['Essential Care Plan'];
  }

  getCurrentPremium(): number {
    const detail = this.getPlanDetail();
    if (!detail) return 0;
    return this.billingCycle === 'monthly' ? detail.monthlyPremium : detail.yearlyPremium;
  }
  ...
  getSavingsPercentage(): number {
    ...
    return Math.round(((monthlyTotal - detail.yearlyPremium) / monthlyTotal) * 100);
  }

  getCoveragePercentage(): number {
    const detail = this.getPlanDetail();
    if (!detail) return 0;
    return Math.min(100, Math.round((detail.coverageUtilization / 100) * 100));
  }
  ...
  navigateToRegister() {
    if (this.currentPlan) {
      ...
      this.router.navigate(['/auth/register'], {
        queryParams: { planId: this.currentPlan.planId }
      });
    }
  }

  switchTab(tab: 'overview' | 'coverage' | 'analytics' | 'exclusions' | 'addons') {
    this.activeTab = tab;
    if (tab === 'analytics') {
